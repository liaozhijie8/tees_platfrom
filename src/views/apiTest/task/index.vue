<template>
  <div class="app-container">
    <el-row>
      ...
      <!-- 服务树 -->
      <el-col style="width: 17%; border:1px solid;border-color: #ffffff rgb(234, 234, 234) #ffffff #ffffff;">
        <!-- 服务列表树组件 -->
        <projectTreeView
          ref="projectTree"
          :busEventClickTree="$busEvents.api.apiProjectTreeChoiceProject"
          :busEventClickMenu="$busEvents.api.apiTaskDrawerIsShow"
          :menuName="'添加任务'"
          :labelWidth="10"
        ></projectTreeView>
      </el-col>
        ...
        <el-tabs v-model="taskTab" class="table_padding" style="margin-left: 5px">
          <el-tab-pane label="定时任务列表" :name="taskTab">
            ...
            >
              ...
              <el-table-column prop="cron" label="cron表达式" min-width="35%">
                <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    editTask(row) {
      if (row.status === 0) {
        this.$bus.$emit(this.$busEvents.api.apiTaskDrawerIsShow, 'update', JSON.parse(JSON.stringify(row)))
      }
    },
  ...
  beforeDestroy() {
    ...
    this.$bus.$off(this.$busEvents.api.apiTaskDrawerIsCommit)
